...
def inference(input):
    temp_input = layers.Input(shape=[416,416,3])
    temp_logit = darknet19(temp_input)

    #init darknet19 to extra feature
    vision_model = Model(temp_input,temp_logit,'darknet19')
    load_weight(vision_model)
    vision_model.trainable = False

    #get hight level feature
    x = vision_model(input)
    # Layer 23
